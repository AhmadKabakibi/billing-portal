...
module.exports = function (apiRouter) {
  ...
  apiRouter.put('/po/accept', function (req, res) {
    return exportService.acceptPOslist(req.body.POList,req.user.id).then(function (result) {
      return successHandler(res, result);
    }).catch(function (error) {
      return errorHandler(res, error);
    });
  ...
  apiRouter.put('/po/reject', function (req, res) {
    return exportService.rejectedPO(req.body.PONumber,req.user.id).then(function (result) {
  ...
  apiRouter.put('/po/inovice', function (req, res) {
    ...
    return exportService.checkPOStatus(req.body.PONumber).then(function (po) {
      if (po.POStatus == 'UnderReview') {
        ...
        exportService.invociePO(req.body.PONumber,req.user.id,req.body.ActionNote).then(function (result) {
          return successHandler(res, result);
        })
